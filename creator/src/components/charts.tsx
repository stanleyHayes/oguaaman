...
export function BarsH({ data, formatValue }: Readonly<{ data: Datum[]; formatValue?: (v: number) => string }>) {
    ...
    <div className="space-y-2.5">
      {data.map((d, i) => (
        <div key={d.label} className="flex items-center gap-3">
          ...
          <span className="shrink-0 text-right text-xs font-semibold tabular-nums text-ink">{formatValue ? formatValue(d.value) : d.value}</span>
        </div>
      ))}
    </div>
  );
