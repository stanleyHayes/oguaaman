...
export function GenrePicker({ value, onChange }: Readonly<{ value: string[]; onChange: (value: string[]) => void }>) {
  ...
  return <fieldset className="space-y-3">
    ...
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">{options.map(([genre, mark]) => {
      const selected = value.some((item) => item.toLowerCase() === genre.toLowerCase());
      return <button type="button" key={genre} aria-pressed={selected} onClick={() => toggle(genre)} className={`flex min-h-14 items-center gap-3 rounded-xl border px-3 py-2 text-left transition ${selected ? "border-green bg-green/[0.09] text-green-text ring-1 ring-green/20" : "border-sand bg-paper text-ink-muted hover:border-green/35"}`}><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-[0.62rem] font-bold ${selected ? "bg-green text-on-green" : "bg-cream text-ink-faint"}`}>{selected ? "✓" : mark}</span><span className="text-sm font-semibold leading-tight">{genre}</span></button>;
    })}</div>
  </fieldset>;
}
