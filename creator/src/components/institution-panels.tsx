...
export function ProfileForm({ slug, org }: Readonly<{ slug: string; org: Organization }>) {
    ...
    <Panel title="Profile">
      <div className="space-y-4">
        ...
        <div>
          ...
          <input id="org-motto" className={field} value={motto} onChange={(e) => setMotto(e.target.value)} placeholder="e.g. Dwen Hwe Kan" />
        </div>
        <div>
          <label htmlFor="org-summary" className={label}>Summary</label>
          <textarea id="org-summary" rows={3} className={`resize-none ${field}`} value={summary} onChange={(e) => setSummary(e.target.value)} />
        </div>
        <div>
...
export function RosterForm({ slug, initial }: Readonly<{ slug: string; initial?: Office[] }>) {
...
export function EventForm({ slug, verified }: Readonly<{ slug: string; verified: boolean }>) {
  ...
  const [eventFormat, setEventFormat] = useState("community"); const [audience, setAudience] = useState<string[]>(["all-ages"]); const [admission, setAdmission] = useState<EventAdmission>("free"); const [tiers, setTiers] = useState<EventTierDraft[]>([]);
  const [state, setState] = useState<SaveState>("idle");
  ...
  async function post() {
    ...
    setMsg("");
    try {
      const fd = new FormData(formRef.current ?? undefined); const value = (key: string) => String(fd.get(key) ?? "").trim(); const details: Record<string, unknown> = { startsAt, ...(endsAt ? { endsAt } : {}), venue, description, eventFormat, audience, admission };
      ...
      details.highlights = value("highlights").split(/\r?\n/).map((item) => item.trim()).filter(Boolean); details.featuredGuests = value("featuredGuests").split(/\r?\n/).map((item) => item.trim()).filter(Boolean);
    ...
    <Panel title="Post an official event">
      <form ref={formRef} className="space-y-4" onSubmit={(e) => { e.preventDefault(); void post(); }}>
        <div>
          ...
          <input id="org-event-title" className={field} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Speech & Prize-Giving Day" />
        </div>
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            <label htmlFor="org-event-date" className={label}>Start date</label>
            <DatePicker id="org-event-date" className="w-full" value={startsAt} onChange={(value) => { setStartsAt(value); if (endsAt && endsAt < value) setEndsAt(""); }} />
          </div>
          <div>
            <label htmlFor="org-event-end-date" className={label}>End date <span className="text-ink-faint">(optional)</span></label>
            <DatePicker id="org-event-end-date" className="w-full" value={endsAt} onChange={setEndsAt} min={startsAt || undefined} placeholder="Same day" />
          </div>
          <div>
            <label htmlFor="org-event-venue" className={label}>Venue</label>
            <input id="org-event-venue" className={field} value={venue} onChange={(e) => setVenue(e.target.value)} placeholder="e.g. Botwe Hall" />
          </div>
        </div>
        <div>
          <label htmlFor="org-event-details" className={label}>Details</label>
          <textarea id="org-event-details" rows={3} className={`resize-none ${field}`} value={description} onChange={(e) => setDescription(e.target.value)} />
        </div>
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={state === "saving" || !title.trim()} className="rounded-full bg-gold-brand px-5 py-2.5 text-sm font-semibold text-green-900 transition-colors hover:bg-gold disabled:opacity-60">
...
export function SectionBuilderForm({ slug, initial }: Readonly<{ slug: string; initial?: ProfileSection[] }>) {
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {SECTION_TYPES.map((st) => (
            <button key={st.type} type="button" onClick={() => addType(st.type)} title={st.hint} className="rounded-full border border-sand bg-paper px-3.5 py-1.5 text-sm font-medium text-ink transition-colors hover:border-green/40">+ {st.label}</button>
          ))}
        </div>
      </div>

      <div className="mt-5 flex items-center gap-3 border-t border-dashed border-sand pt-4">
        <button type="button" onClick={save} disabled={state === "saving"} className={btnPrimary}>Save sections</button>
        <Saver state={state} />
      </div>
    </Panel>
  );
...
function ItemsEditor({ type, items, onChange }: Readonly<{ type: ProfileSectionType; items: SectionItem[]; onChange: (items: SectionItem[]) => void }>) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button type="button" onClick={add} className={btnGhost}>+ Add item</button>
    </div>
  );
}
...
function MediaItemsEditor({ media, onChange }: Readonly<{ media: MediaAsset[]; onChange: (media: MediaAsset[]) => void }>) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button type="button" onClick={add} className={btnGhost}>+ Add photo</button>
    </div>
  );
}
...
function GroupsEditor({ groups, onChange }: Readonly<{ groups: SubEntity[]; onChange: (groups: SubEntity[]) => void }>) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button type="button" onClick={add} className={btnGhost}>+ Add card</button>
    </div>
  );
}
...
function AttrsEditor({ attrs, onChange }: Readonly<{ attrs: SectionItem[]; onChange: (attrs: SectionItem[]) => void }>) {
  ...
  return (
    <div className="space-y-2">
      ...
      <button type="button" onClick={add} className={btnGhost}>+ Add fact</button>
    </div>
  );
}

export function GalleryForm({ slug, initial }: Readonly<{ slug: string; initial?: MediaAsset[] }>) {
  ...
  return (
    <Panel title="Photo gallery">
      ...
      <div className="mt-4 flex items-center gap-3">
        <button type="button" onClick={add} className={btnGhost}>+ Add photo</button>
        <button type="button" onClick={save} disabled={state === "saving"} className={btnPrimary}>Save gallery</button>
        <Saver state={state} />
      </div>
    </Panel>
  );
