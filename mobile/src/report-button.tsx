    ...
    <View style={s.panel}>
      {state === "done" ? (
        <>
          ...
          <Pressable onPress={() => setOpen(false)}><Text style={s.cancel}>Close</Text></Pressable>
        </>
      ) : (
        <>
          ...
          <View style={s.reasons}>
            {reasons.map((r) => (
              <Pressable key={r.value} onPress={() => setReason(r.value)} style={[s.chip, reason === r.value && s.chipOn]}>
          ...
          <View style={s.actions}>
            <Pressable onPress={() => setOpen(false)}><Text style={s.cancel}>Cancel</Text></Pressable>
            <Pressable onPress={submit} disabled={state === "sending"} style={[s.send, state === "sending" && { opacity: 0.6 }]}>
