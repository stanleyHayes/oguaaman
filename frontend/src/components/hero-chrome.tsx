...
// Shared banner chrome for PageHero / DetailHero: a route-derived breadcrumb
// trail, a mount-animated section icon, and a large faint corner watermark.
// Route derivation lives in lib/breadcrumbs.ts so this file stays components-only.
...
export function Breadcrumbs({ crumbs, onDark = false, className = "" }: Readonly<{ crumbs: Crumb[]; onDark?: boolean; className?: string }>) {
...
export function HeroIcon({ sectionId, onDark = false, toneText, className = "" }: Readonly<{ sectionId: string; onDark?: boolean; toneText?: string; className?: string }>) {
  const reduce = useReducedMotion();
  const tile = onDark
    ? "border-gold/40 bg-gold/15 text-gold"
    : `border-gold-border/40 bg-gold/[0.12] ${toneText ?? "text-gold-text"}`;
  return (
    <motion.span
...
export function HeroWatermark({ sectionId, onDark = false, toneText }: Readonly<{ sectionId: string; onDark?: boolean; toneText?: string }>) {
  const color = onDark ? "text-gold" : (toneText ?? "text-gold-text");
