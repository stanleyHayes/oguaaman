...
// Mirrors the app's inputCls idiom (rounded-xl border-sand bg-cream text-ink with
// the gold focus ring), sized as a square-ish digit box. Theme-aware via tokens.
const boxCls =
  "h-12 w-11 rounded-xl border border-sand bg-cream text-center text-lg font-semibold text-ink transition-colors focus:border-gold-border focus:bg-paper focus:outline-none focus:ring-2 focus:ring-gold/20";
...
export function OtpInput({
  ...
  const handleKeyDown = (index: number, e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "Backspace") {
      e.preventDefault();
      if (chars[index]) {
        // Remove only the focused slot; keep the rest and focus in place.
        emit(chars.slice(0, index).join("") + chars.slice(index + 1).join(""));
      } else if (index > 0) {
        // Empty slot — remove the previous digit and step focus back.
        emit(chars.slice(0, index - 1).join("") + chars.slice(index).join(""));
        focusBox(index - 1);
      }
