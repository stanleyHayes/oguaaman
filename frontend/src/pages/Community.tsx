    ...
    <>
      ...
      <section className="relative overflow-hidden border-b border-sand bg-paper py-16 sm:py-24">
        ...
        <Container className="relative grid gap-10 lg:grid-cols-[0.7fr_1.3fr]">
          ...
          <Reveal delay={0.08}><p className="max-w-[44rem] text-lg font-medium leading-[1.65] text-ink sm:text-xl">Community is not a category on Oguaa. It is the engine: one generation holding the door open for the next, while keeping the names and stories behind us in view.</p><div className="mt-8 flex flex-wrap gap-x-8 gap-y-3 border-t border-sand pt-6 text-sm text-ink-muted"><span><strong className="mr-2 text-2xl text-green tabular-nums">{memories.length}</strong> memories held</span><span><strong className="mr-2 text-2xl text-green tabular-nums">{opps.length}</strong> open doors</span><span><strong className="mr-2 text-2xl text-green tabular-nums">{events.length}</strong> upcoming highlights</span></div></Reveal>
        </Container>
      </section>
