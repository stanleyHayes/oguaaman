...
import { Container, CTA, Avatar } from "@/components/ui";
  ...
  return (
    <>
      <section className="on-dark bg-green text-cream">
        <Container className="flex flex-col gap-5 py-12 sm:flex-row sm:items-center">
          <Avatar initials={me.initials} photoUrl={photo} size={72} className="border-2 border-gold/40" />
          <div>
            <p className="text-xs uppercase tracking-wide text-gold/90">{me.role === "curator" ? "Curator" : me.role === "steward" ? "Steward" : "Member"} · joined {formatDate(me.joinedAt)}</p>
            ...
            <p className="mt-3 inline-flex items-center gap-1.5 text-xs text-cream/60">
      ...
      <Container className="space-y-12 py-12">
        <section>
          ...
          <p className="mt-1 text-sm text-ink-muted">Put a face to your name. It shows on your profile and across the community.</p>
        ...
        <section>
          <h2 className="font-display text-2xl font-semibold text-ink">Rep your quarter</h2>
          <p className="mt-1 text-sm text-ink-muted">Wear your community pride. {town && <>You rep <b>{town.name}</b>.</>}</p>
          <div className="mt-4 flex flex-wrap gap-2">
            {quarters.map((p) => (
              <button key={p.id} type="button" onClick={() => chooseQuarter(p.id)} className={`rounded-full border px-3.5 py-1.5 text-sm transition-colors ${p.id === townId ? "border-green bg-green text-cream" : "border-sand bg-cream text-ink-muted hover:border-green/40"}`}>{p.name}</button>
            ))}
          </div>
        </section>

        <section>
          <h2 className="font-display text-2xl font-semibold text-ink">Rep your Asafo</h2>
          <p className="mt-1 text-sm text-ink-muted">Oguaa&rsquo;s seven companies — choose the one you belong to.</p>
          <div className="mt-4 flex flex-wrap gap-2">
            {asafos.map((p) => (
              <button key={p.id} type="button" onClick={() => chooseAsafo(p.id)} className={`inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 text-sm transition-colors ${p.id === asafoId ? "border-clay bg-clay text-cream" : "border-sand bg-cream text-ink-muted hover:border-clay/40"}`}>
        ...
        <section>
          <h2 className="font-display text-2xl font-semibold text-ink">Your birthday</h2>
          <p className="mt-1 text-sm text-ink-muted">If you turn this on, your followers get a gentle note on your day. Off by default — it&rsquo;s yours to choose.</p>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <input
              type="date"
              value={birthday.length >= 10 ? birthday.slice(0, 10) : birthday}
              onChange={(e) => { setBirthday(e.target.value); setBdayState("idle"); }}
              className="rounded-md border border-sand bg-cream px-3 py-2 text-sm text-ink focus:border-gold-brand focus:outline-none"
            />
        ...
        <section>
          <h2 className="font-display text-2xl font-semibold text-ink">Oguaa abroad</h2>
          <p className="mt-1 text-sm text-ink-muted">A son or daughter of the Castle living away from home? Add yourself to the diaspora — the bridge for homecomings, projects, and giving back. Off by default.</p>
          <div className="mt-4 space-y-3">
            ...
            {abroad && (
              <div className="flex flex-wrap gap-3">
                <input value={city} onChange={(e) => { setCity(e.target.value); setDiaState("idle"); }} placeholder="City (e.g. London)" className="rounded-md border border-sand bg-cream px-3 py-2 text-sm text-ink focus:border-teal focus:outline-none" />
                <input value={country} onChange={(e) => { setCountry(e.target.value); setDiaState("idle"); }} placeholder="Country (e.g. United Kingdom)" className="rounded-md border border-sand bg-cream px-3 py-2 text-sm text-ink focus:border-teal focus:outline-none" />
              </div>
            )}
        ...
        <section>
          <h2 className="font-display text-2xl font-semibold text-ink">Your schooling</h2>
          <p className="mt-1 text-sm text-ink-muted">Add the schools you attended and the years you were there. Classmates who overlapped with you will show up below — this is the powerhouse of Oguaa&rsquo;s networks.</p>
          <SchoolingEditor
            schools={schools}
        ...
        <section>
          <h2 className="font-display text-2xl font-semibold text-ink">People you may know</h2>
          <p className="mt-1 text-sm text-ink-muted">Classmates, neighbours from your quarter, and members of your Asafo.</p>
          <PeopleYouMayKnow key={connKey} />
        </section>

        <section>
          <h2 className="font-display text-2xl font-semibold text-ink">My tickets</h2>
          <p className="mt-1 text-sm text-ink-muted">Your event tickets and gate codes. Show the code at the entrance.</p>
          <ul className="mt-5 divide-y divide-sand overflow-hidden rounded-[var(--radius-card)] border border-sand bg-cream">
            {tickets.map((t) => (
              <li key={t.id}>
                <Link to={`/events/${t.eventSlug}`} className="flex items-center gap-3 px-5 py-4 hover:bg-paper">
            ...
            {tickets.length === 0 && <li className="px-5 py-8 text-center text-sm text-ink-muted">No tickets yet. <Link to="/events" className="font-semibold text-green">See what&rsquo;s on →</Link></li>}
          </ul>
        </section>

        <section>
          <h2 className="font-display text-2xl font-semibold text-ink">My subscriptions</h2>
          <p className="mt-1 text-sm text-ink-muted">Your Supporter subscriptions — each payment adds a month to the business.</p>
          <ul className="mt-5 divide-y divide-sand overflow-hidden rounded-[var(--radius-card)] border border-sand bg-cream">
            {subscriptions.map((s) => (
              <li key={s.id}>
                <Link to={`/business/${s.listingSlug}`} className="flex items-center gap-3 px-5 py-4 hover:bg-paper">
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-medium text-ink">{s.listingTitle}</p>
                    <p className="text-xs text-ink-faint">
                      GH₵ {(s.amountPesewas / 100).toLocaleString("en-GH", { maximumFractionDigits: 2 })}
                      {s.periodEnd ? ` · until ${formatDate(s.periodEnd)}` : ""}
                    </p>
                  </div>
                  <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${s.status === "success" ? "bg-green/[0.08] text-green" : s.status === "pending" ? "bg-gold/[0.14] text-gold-text" : "bg-maroon-900/[0.08] text-maroon-900"}`}>{s.status}</span>
                </Link>
              </li>
            ))}
            {subscriptions.length === 0 && <li className="px-5 py-8 text-center text-sm text-ink-muted">No subscriptions yet.</li>}
          </ul>
        </section>

        <section>
          <div className="flex items-center justify-between">
            <h2 className="font-display text-2xl font-semibold text-ink">Your listings</h2>
            <CTA to="/submit" variant="outline">+ Add a listing</CTA>
          </div>
          <p className="mt-1 text-sm text-ink-muted">Promote an approved listing to feature it on the front pages — GH₵ 10 per day.</p>
          {promoConfirmed && (
            <p className="mt-4 rounded-lg bg-green/[0.08] px-4 py-3 text-sm font-medium text-green">
              Payment confirmed — &ldquo;{promoConfirmed.listingTitle}&rdquo; is now featured for {promoConfirmed.days} days. ✓
            </p>
          )}
          {promoError && (
            <p className="mt-4 rounded-lg bg-maroon-900/[0.08] px-4 py-3 text-sm font-medium text-maroon-900">{promoError}</p>
          )}
          <ul className="mt-5 divide-y divide-sand overflow-hidden rounded-[var(--radius-card)] border border-sand bg-cream">
            {listings.map((l) => {
              ...
              return (
                <li key={l.id}>
                  <div className="flex items-center gap-2 pr-3">
                    {href ? <Link to={href} className="block min-w-0 flex-1 hover:bg-paper">{inner}</Link> : <div className="min-w-0 flex-1">{inner}</div>}
            ...
            {listings.length === 0 && <li className="px-5 py-8 text-center text-sm text-ink-muted">Nothing yet. <Link to="/submit" className="font-semibold text-green">Add your first →</Link></li>}
          </ul>
        </section>
      </Container>
    </>
