...
export function Component() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await api.reportIncident({
        ...
        category: String(fd.get("category")) as IncidentCategory,
        severity: String(fd.get("severity")) as IncidentSeverity,
        location: String(fd.get("location") ?? "").trim(),
        contact: String(fd.get("contact") ?? "").trim(),
        description: String(fd.get("description") ?? "").trim(),
      });
      navigate(`/safety/${created.slug}`);
    } catch (err) {
  ...
  return (
    <>
      <PageHero tone="maroon" kicker="Safety" title="Report an incident" symbol="dwennimmen" lede="Floods, fires, accidents, hazards — post what is happening so responders and neighbours can act. It goes live immediately; a curator verifies afterwards. In a life-threatening emergency, call the emergency services first." />
        ...
        <div>
          {member ? (
            <form onSubmit={onSubmit} className="space-y-5 rounded-[var(--radius-card)] border border-sand bg-cream p-6 sm:p-8">
              ...
              <Field label="Location" hint="As precise as you can — a landmark, a street, a compound.">
                <input name="location" required className={inputCls} placeholder="e.g. Kotokuraba Market, near the main gate" />
              </Field>
              <Field label="What happened" hint="What responders and neighbours need to know.">
            ...
            <div className="rounded-[var(--radius-card)] border border-sand bg-cream p-8 text-center">
              <h2 className="font-display text-2xl font-semibold text-ink">Sign in to report</h2>
        ...
        <aside className="space-y-6">
          <div className="rounded-[var(--radius-card)] border border-sand bg-cream p-5">
            ...
            <ol className="mt-3 space-y-3 text-sm text-ink-muted">
              {[
                ["Reported", "Your report goes live immediately — time matters."],
          ...
          <div className="rounded-[var(--radius-card)] border border-dashed border-sand p-5 text-sm text-ink-faint">
            In a life-threatening emergency, call the Ghana emergency services (112 / 191 fire / 193 police) first — this page is for alerting the community, not a replacement for the services.
          </div>
        </aside>
      </Container>
    </>
