// The official-page editor, embedded in the admin: configure an institution's
...
function ProfileForm({ slug, org }: Readonly<{ slug: string; org: Organization }>) {
  ...
  return (
    <Panel title="Summary & history" hint="The lead description and the longer history shown at the top of the public page.">
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <button type="button" onClick={save} disabled={state === "saving"} className="rounded-full bg-green px-5 py-2.5 text-sm font-semibold text-on-green hover:bg-green-900 disabled:opacity-60">Save summary & history</button>
          <Saver state={state} />
        </div>
      </div>
    </Panel>
...
const SECTION_TYPES: { type: ProfileSectionType; label: string; hint: string }[] = [
  ...
  { type: "gallery", label: "Photo album", hint: "A titled set of photos." },
  { type: "stats", label: "Facts", hint: "Key numbers — built, hours, distance." },
  { type: "team", label: "People", hint: "Custodians, guides, or notable figures." },
  { type: "timeline", label: "Timeline", hint: "Dated milestones." },
  { type: "faq", label: "FAQ", hint: "Plan-your-visit questions & answers." },
  { type: "docs", label: "Downloads", hint: "Links to documents — maps, guides." },
  { type: "quote", label: "Quote", hint: "A pull-quote; the section title is the attribution." },
  { type: "cta", label: "Call to action", hint: "Heading + text + buttons (book, visit)." },
  { type: "logos", label: "Partners", hint: "A wall of logos — partners, funders." },
...
function defaultSectionTitle(type: ProfileSectionType): string {
  const titles: Record<ProfileSectionType, string> = {
    richtext: "About", gallery: "Photos", stats: "Visiting facts",
    team: "Custodians", timeline: "Through the years", faq: "Plan your visit", docs: "Downloads",
    quote: "", cta: "Plan your visit", logos: "Partners", divider: "", groups: "Houses",
...
const ITEM_FIELDS: Record<string, { key: ItemFieldKey; placeholder: string; textarea?: boolean }[]> = {
  stats: [
    { key: "label", placeholder: "Label (e.g. Built)" },
    { key: "value", placeholder: "Value (e.g. 1653)" },
  ],
  team: [
    { key: "value", placeholder: "Name" },
    { key: "label", placeholder: "Role" },
    { key: "detail", placeholder: "Short note", textarea: true },
  ],
  timeline: [
    { key: "label", placeholder: "Date (e.g. 1653)" },
  ...
  docs: [
    { key: "label", placeholder: "Title (e.g. Visitor map)" },
    { key: "detail", placeholder: "Note (e.g. PDF)" },
    { key: "url", placeholder: "https://link-to-file" },
  ],
  cta: [
    { key: "label", placeholder: "Button text (e.g. Book a tour)" },
...
function SectionBuilderForm({ slug, initial }: Readonly<{ slug: string; initial?: ProfileSection[] }>) {
  ...
  return (
    <Panel title="Sections" hint="Build the page — facts, a timeline, a photo album, an FAQ and more. Add sections below and order them with the arrows.">
      <div className="space-y-4">
        {sections.map((s, i) => (
          <div key={s.id || i} className="rounded-[var(--radius-card)] border border-sand bg-paper p-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => move(i, -1)} disabled={i === 0} aria-label="Move up" className="rounded-md border border-sand px-2 py-1.5 text-sm text-ink-muted hover:border-green-text/40 disabled:opacity-40">↑</button>
                <button type="button" onClick={() => move(i, 1)} disabled={i === sections.length - 1} aria-label="Move down" className="rounded-md border border-sand px-2 py-1.5 text-sm text-ink-muted hover:border-green-text/40 disabled:opacity-40">↓</button>
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {SECTION_TYPES.map((st) => (
            <button key={st.type} type="button" onClick={() => addType(st.type)} title={st.hint} className="rounded-full border border-sand bg-paper px-3.5 py-1.5 text-sm font-medium text-ink hover:border-green-text/40">+ {st.label}</button>
          ))}
        </div>
      </div>
      ...
      {items.map((it, i) => (
        <div key={it.id || i} className="rounded-lg border border-sand bg-cream p-3">
          <div className="space-y-2">
            ...
            {showImage && (
              <ImageUpload value={it.image ?? ""} onChange={(url) => update(i, { image: url })} label="Photo (optional)" hint="Shown beside the name." />
            )}
          </div>
          <button type="button" onClick={() => remove(i)} className="mt-2 text-sm text-clay-text hover:underline">Remove</button>
        </div>
      ))}
      <button type="button" onClick={add} className="rounded-full border border-green-text/30 px-4 py-2 text-sm font-semibold text-green-text hover:border-green-text">+ Add item</button>
    </div>
  );
}
    ...
    <div className="space-y-3">
      {media.map((m, i) => (
        <div key={m.id || i} className="rounded-lg border border-sand bg-cream p-3">
          <ImageUpload value={m.url} onChange={(url) => update(i, { url })} label={`Photo ${i + 1}`} hint="Shown in this album on the public page." />
          <input className={`${field} mt-2`} value={m.caption ?? ""} onChange={(e) => update(i, { caption: e.target.value })} placeholder="Caption (e.g. The Door of No Return)" />
      ...
      <button type="button" onClick={add} className="rounded-full border border-green-text/30 px-4 py-2 text-sm font-semibold text-green-text hover:border-green-text">+ Add photo</button>
    </div>
  );
}
...
function GroupsEditor({ groups, onChange }: Readonly<{ groups: SubEntity[]; onChange: (groups: SubEntity[]) => void }>) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button type="button" onClick={add} className="rounded-full border border-green-text/30 px-4 py-2 text-sm font-semibold text-green-text hover:border-green-text">+ Add card</button>
    </div>
  );
}
...
function AttrsEditor({ attrs, onChange }: Readonly<{ attrs: SectionItem[]; onChange: (attrs: SectionItem[]) => void }>) {
  ...
  return (
    <div className="space-y-2">
      ...
      <button type="button" onClick={add} className="rounded-full border border-green-text/30 px-3.5 py-1.5 text-sm font-semibold text-green-text hover:border-green-text">+ Add fact</button>
    </div>
  );
}
...
function GalleryForm({ slug, initial }: Readonly<{ slug: string; initial?: MediaAsset[] }>) {
  ...
  return (
    <Panel title="Photo gallery" hint="Add photos of this place. Each can carry a caption and a short description (alt text) for accessibility.">
      <div className="space-y-3">
        {items.map((m, i) => (
          <div key={m.id || i} className="rounded-lg border border-sand bg-paper p-3">
            <ImageUpload value={m.url} onChange={(url) => update(i, { url })} label={`Photo ${i + 1}`} hint="Shown in the gallery on the public page." />
