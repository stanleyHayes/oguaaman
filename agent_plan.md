...
21. **Dark theme rollout** — Phase A ✅ (`[data-theme="dark"]` CSS variable maps for paper/cream/sand/ink/gold/clay/teal + shadows; no per-component hardcoded dark colors), Phase B ✅ (app-shell toggle in frontend/admin/creator headers; persisted to `localStorage` with OS-preference fallback; inline script sets `data-theme` before first paint to prevent flash). Remaining: Phase C component pass (cards/forms/modals/date-picker hardcoded surfaces), Phase D mobile token equivalents, Phase E contrast audit.
