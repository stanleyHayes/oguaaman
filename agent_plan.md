...
| §17.3 | Edit re-approval | **Minor/major split shipped**: major content/title edits re-enter moderation (`owner-edit-major`), link/hours/contact/image edits stay live (`owner-edit-minor`) | Creator Plan decision + shipped implementation, 2026-07. |
...
Backend additions: `GET /api/admin/{members,listings,institutions,audit}`, `POST /api/admin/members/:id/{role,suspend}`, `POST /api/admin/institutions/:id/verify`, `POST /api/admin/listings/:id/unpublish`. (Auth gating delivered: `/api/admin/*` is role-guarded — curator/steward pass, plus a scoped `moderator` role limited to queue/listings/reports/incidents; staff are force-enrolled into TOTP MFA before the console unlocks. Admin SPA moderator gate/nav filtering is now shipped — see §7.4 #15.)
...
*Full audit of all three spec documents against the codebase, 2026-07-16. Update this section whenever a feature ships — it is the canonical done/left ledger. Statuses: ✅ done · ◐ partial (what remains in italics) · ☐ not started.*
...
- ◐ §8.11 **Memorial keeper controls** — *fixed: submit defaults `remindersEnabled` on / `observeBirthday` off (explicit choices respected), the owner-edit whitelist accepts both flags, omitted flags carry over instead of resetting, and the portal submit form + creator editor expose the toggles. Keeper-claim workflow now delivered (`POST /api/memorials/{slug}/keeper-claim` + curator grant endpoint and admin report action). Still open: broader family claim/correct/remove variants and funeral/celebration-of-life details (Open Decision #7). Reminders now mirror out-of-band via `notifyOutOfBand` (Resend email + WhatsApp) when the member has contact channels.*
...
**Partial / open:**
- ◐ **Per-kind structured catalog (§4)** — partially delivered (`gesCategory`, `boardingType`, `genderPolicy`, `nhisAccredited`, `ghanaPostGPS`, `momoNumber`) across backend + portal/admin/creator editors. Remaining: deeper office-holder lineage, OSA endowment, dual-tier pricing, and other kind-specific verification artifacts.
...
- ◐ **Notices** — *official events still double as announcements; no distinct notice type yet. Mobile institution pages now render Events & Announcements from `officialEvents`.*
- ◐ **§7 Localization pack** — GhanaPostGPS + lat/long + quarter/Asafo tags + verification artifacts are now wired through backend persistence and portal/admin/creator/mobile types/editors; still open: field-level i18n catalog wiring, MoMo giving flows, and deeper per-kind structured verification packs.
...
- ✅ **Manager revocation** endpoint/UI — *team roster + remove action in admin Institution detail, backed by `DELETE /api/institutions/{slug}/team/{memberId}`.*
...
**Phase 3 (visibility) — partially delivered.** View counter (`POST /api/listings/:id/view`, daily-deduped) ✅; "Trade in Oguaa" home band (category chips + 4 cards, supporters first) ✅; creator CTA banner ✅ (`/me` "Become a creator" entry panel for members with no `creatorTypes`). Remaining: creator public profiles `/creators/:slug` *(marked "?" in the plan — still undecided)* and deeper creator analytics beyond `viewsThisMonth`.

**Small gaps:** Payouts ledger; creatorTypes migration for pre-existing business owners; `dev.sh` verify script; citizen upgrade screen is still a soft CTA (not the originally planned dedicated flow).

### 7.4 Prioritized backlog (next-up order)

1. ~~OTP verification + submit gate~~ ✅ (`d97c11c` core slice — codes, gate, UI + later WhatsApp provider wiring).
...
11. ~~**Email/WhatsApp notification delivery**~~ ✅ (spec §8.11/§12) — `notifyOutOfBand` mirrors in-app notifications to Resend email + WhatsApp Cloud API when the member has reachable channels; call sites: remembrance reminders (§8.11), moderation approve/reject outcomes, owner report alerts, org team invites; OTP rides the same WhatsApp client. Noop fallbacks when `RESEND_API_KEY`/`WHATSAPP_TOKEN` unset; covered by `outbound_notifications_test.go`; `.env.example` documents the real env vars.
12. ~~**Maps depth**~~ ✅ — geocoded per-listing pins + mobile map parity (web Leaflet map cards now geocode + pin; mobile LocationCard parity shipped on key detail pages).
13. ~~**Institution localization pack**~~ ◐ — lat/long, quarter/Asafo tags, GhanaPostGPS and verification-artifact fields wired through backend + portal/admin/creator/mobile surfaces; remaining: field-level i18n + deeper per-kind structured packs.
...
21. **Dark theme rollout** — Phase A ✅, Phase B ✅, **Phase C ✅** (component/page pass across frontend/admin/creator: added `--color-on-green`/`--color-green-text`/`--color-maroon-text` foreground tokens plus an `.on-dark-pin` subtree utility with a `.theme-surface` counter-pin for always-dark stages (heroes, header, footer, featured cards, lightbox, boot screen); swept every `text-cream`-on-green/maroon fill → `text-on-green` and colored foreground → the `-text` tokens; chart palettes and metric-card accents point at CSS vars; AI-bar white surfaces tokenised; auth-gate brand panels re-pinned), **Phase D ✅** (mobile `theme.ts` LIGHT/DARK palettes mirroring the web tokens, a persisted `ThemeProvider` under `oguaa.theme` with `useColorScheme` fallback, a System/Light/Dark toggle in settings, and all screens migrated to `useTheme()`), **Phase E ✅** (contrast verified in-browser across new surfaces + a multi-agent dark-theme sweep). Also: **circular-reveal theme toggle** (View Transitions API, reduced-motion fallback).
22. **Brand — Oguaa crab mark** ✅ — a single-line Kotokuraba crab drawn on a `0 0 64 64` grid in `currentColor`; one geometry serves the app icon, favicon, inline wordmarks and the admin/creator sidebar marks; replaced the stray placeholder favicon.
23. **Auth & profile UX** ✅ — 3-step sign-up wizard (`/signin?mode=join`); password show/hide toggles everywhere; 6-box OTP input with a recovery-code fallback; MFA QR branded with the crab; branded loading splash across admin/creator; birthday pre-filled on `/me` from the sign-up date of birth; `/me` reorganised into tabbed sections.
24. **Alerts / authorized directives** ✅ (spec §8.13 official announcements) — authority institution kinds (`emergency-service`/`security-service`/`health-service`/`local-government`), reserved official handles (steward-assigned only), steward verification before an authority can act, and directives with severity + a timed effective window. High/critical directives broadcast townwide and, on web, ring (Web Audio) + vibrate; a sticky alert banner with live countdown + an `/alerts` page; admin authoring/cancel; mobile poll + vibrate. Seeds Cape Coast Fire & Rescue + the Metropolitan Assembly (local-government).
25. **Verified badges** ✅ — computed `verified`/`verifiedAs` on member profiles (curator/steward, or an approved manager of a verified authority org); shown on profiles, news bylines and institution pages across web + mobile.
26. **Creator platform** ✅ — profile editing (photo/bio/links) on the creator dashboard; creator-type-gated tool panels; a new `writer` creator type with a Write surface that submits news (drafts to the newsroom, auto-publish for verified authority managers). Offloads news authoring to writers + authorised institutions.
27. **Education roster expansion** ✅ — 31 more Cape Coast schools across basic / SHS / technical-vocational / special / private (classification encodes level + public/private/mission + gender), incl. Aboom A.M.E. Zion 'C' Basic and University Practice SHS.
28. **Sundry UX fixes** ✅ — footers gained social + per-link/section icons and lost the dot texture; nav pills no longer wrap; listing cover images load in dev (`/uploads` proxy); lost & found notices take an optional photo; removed "coming soon" labels; page banners gained breadcrumbs, an animated section icon and a bottom-right page watermark; marketing banners + nav dropdown redesigned to match the portal; seeded-account hints removed; forgot-password reset flow on the admin sign-in (`POST /api/auth/password/reset/{start,confirm}`).
29. **Explore Cape Coast map** ✅ — `GET /api/map` aggregates businesses/events/institutions/schools/incidents/lost&found/landmarks/services/transport as pins (real seeded coordinates, optional lat/lng on listings + directives), plus a heritage walk and the Fetu durbar trail, and active directives as geo areas. Frontend Leaflet map page (`/map`) with layer toggles, tap-a-pin detail, in-app walking routes (OSRM + Google/Apple deep-links, `VITE_ROUTING_URL` overridable), heritage/festival/safety/quarter modes, offline-cached payload, and an owner "pin your location" picker on the submit + institution forms; mobile Explore screen with layer-grouped lists + walking deep-links.
30. **Creator settings** ✅ — a creator Settings page (Account, Preferences with a System/Light/Dark theme control + notification toggles, Security with change-password and MFA enrol/disable) backed by a new authenticated `POST /api/me/password`; mobile gained an equivalent settings screen.
31. **School autocomplete** ✅ — the schooling picker is a type-to-filter `SchoolCombobox` (web + mobile) instead of a long native select.
32. **Pagination** ✅ — non-breaking optional `?page`/`?pageSize` on 14 list endpoints (returns an `{items,total,page,pageSize,totalPages}` envelope when `page` is present, else the unchanged array); reusable Pagination controls / load-more across admin, frontend, creator, and FlatList infinite-scroll on mobile.
33. **Mobile top-bar + parity** ✅ — a shared per-tab action cluster (theme toggle, active-directive-badged alerts bell, search) plus the settings/search/alerts/explore screens; remaining mobile gaps are the creator-studio tooling (Overview/Money/Grow/EditListing/Team/Institutions manage) and a few phase-2 content pages.
34. **Verified business commerce — web/API/admin foundation** ✅ (2026-08-14) — private `business_verifications` records keep Ghana Card, registration documents and settlement details out of public listing JSON; owner submission requires legal identity, registration number, Ghana Card PIN, phone, GhanaPostGPS, at least three secure document URLs, and settlement account details. Curator/steward approval provisions a Paystack subaccount; revoked/rejected businesses cannot accept payments. `commerce_orders` snapshots product lines, discounts, gross, Oguaa platform fee and business net; checkout uses Paystack `subaccount` + exact `transaction_charge` with `bearer=subaccount`, and confirms idempotently via redirect or signed webhook. Owner order fulfilment, buyer order history, admin verification/order operations, and commerce fee revenue are API-backed. The portal product page now takes pickup/delivery orders through Paystack; storefront owners manage KYC, orders and coupons from `/business/:slug/commerce`; admin operates the queue from `/commerce`.
35. **Business + Oguaa promotions & coupons** ✅ (2026-08-14 API/web/mobile) — existing paid featured-placement promotions remain the visibility product. Businesses can create percentage/fixed checkout campaigns; stewards can launch platform-wide Oguaa campaigns. Every promotion declares its owner/funding source: business-funded discounts preserve the original Oguaa commission and reduce business net, while platform-funded discounts reduce Oguaa's fee and preserve business settlement. Checkout resolves and atomically reserves codes before Paystack, and snapshots promotion ID, funding, discount and settlement amounts. Mobile includes buyer coupon/affiliate entry plus a stepped owner workspace (Offers → Programme → Partner → Activity; Oguaa step for stewards) linked from an owned business page. Remaining targeting parity: product-specific campaigns (current scope is storefront or platform-wide).
36. **Automated research desk** ✅ (2026-08-14 API/web/admin/mobile) — a startup + interval background worker reads only operator-allowlisted RSS/Atom feeds, filters for Cape Coast relevance, strips unsafe markup, deduplicates by source URL, and publishes source-linked newsroom summaries marked `Automated`. When a real AI provider is configured it produces a grounded summary from fetched source text and labels it `AI-assisted`; provider simulation is never published. Alerts require a separately configured official-authority feed with explicit org attribution; automation can issue only 24-hour medium advisories and cannot mint high/critical emergencies. News and alert provenance is visible on portal, admin and mobile. Configuration: `AUTO_NEWS_FEEDS`, `AUTO_ALERT_FEEDS`, `AUTO_RESEARCH_INTERVAL_MINUTES`. Next phase: multi-source corroboration, editorial review thresholds, source-health telemetry and an operator feed-management UI.

### 7.5 Affiliate marketing — Phase 1 delivered (Xtiitch-grounded)

The first operational phase is now implemented using Xtiitch's durable concepts, adapted to Mongo and Oguaa's member/listing model:

1. ✅ **Programme ownership and funding** — Oguaa and business programmes, basis-point commission, active state and configurable hold days. Business commission is added to Paystack's exact platform transaction charge so Oguaa holds it for later affiliate payout; platform commission comes from Oguaa's retained fee. Both are snapshotted without changing the customer total.
2. **Affiliate onboarding and trust** — application → review → affiliate account; requested code, creator channels and audience note; unique normalized codes; payout profile held privately; active/paused/rejected status; member identity and verified contact required before payout.
3. ◐ **Attribution** — affiliate `?aff=CODE` links on web and explicit codes on mobile create a unique immutable reservation against the pending order; server-verified Paystack confirmation converts it and starts the hold. *Next: signed click IDs/cookies and privacy-preserving click analytics.*
4. ◐ **Commission ledger** — `reserved → converted → payable → paid | void`, immutable amounts/funding, hold date, business dashboard and steward release/payment controls are delivered. *Next: Paystack Transfer execution, refund/chargeback-triggered automatic reversal and reconciliation jobs; until then `paid` is an operator-recorded manual payout state.*
5. **Fraud controls** — reject self-referrals by member/contact match; velocity alerts for repeated hashed-IP clicks; reversal-rate flags; no commission on demo, owner, staff or duplicate orders; durable audit events for programme, payout-profile and campaign-link mutations.
6. **Surfaces** — business programme/partner controls, affiliate application and portal (links, clicks, conversions, payable balance, payouts), admin growth/risk/payout operations, and customer-facing disclosure. Mobile follows only after the web/API ledger passes real Paystack refund and payout tests.

**Affiliate acceptance gates:** unit/integration tests for attribution uniqueness, self-referral and reversal handling; signed-link tamper tests; real GHS Paystack order + refund evidence; payout reconciliation; Act 843 retention/export/erasure treatment; terms and tax/legal approval. No affiliate balance is labelled payable before the hold window closes.

> **Deployment note.** This working copy runs against a Docker stack whose images can't be rebuilt in the sandbox (no npm/Go-module network). The web containers are served fresh `dist/` and the `api` container runs a swapped-in cross-compiled binary; a real `docker compose build` on a networked machine bakes it all in permanently. In-app walking routes use the public OSRM demo (driving profile) with Google/Apple Maps walking deep-links as the authoritative fallback — set `VITE_ROUTING_URL` to a keyed provider for true foot routing.
